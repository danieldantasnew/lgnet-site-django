...
import { speakMessageClicked } from "./readingMode.js";
import addMatchMedia from "../matchMedia.js";
import { state, resources } from "./values.js";
...
export default function initAcessibilidade() {
  ...
  document.addEventListener("click", (event) => {
    const onBtn = event.target.closest("[data-dark-on]");
    if (onBtn) {
      addDark();
      return;
    }

    const offBtn = event.target.closest("[data-dark-off]");
    if (offBtn) {
      removeDark();
      return;
    }
    ...
    if (
      !event.target.closest('[data-dropdown="acessibilidade-modal"]') &&
      state.readerMode
    ) {
      speakMessageClicked(event);
    }

    resources.forEach((attribute) => {
